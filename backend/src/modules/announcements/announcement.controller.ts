...
export async function listAnnouncements(req: Request, res: Response) {
  try {
    ...
    const filteredAnnouncements = isAdminOrHR 
      ...
      : announcements.filter((announcement) => {
          ...
          if (target.type === "role") {
            ...
            const userRoleIds = currentUser.userRoles.map((ur: any) => ur.role.id);
            const targetIds = target.ids || [];
            // Check if any of the user's roles match the target role IDs
            return targetIds.some((roleId: string) => userRoleIds.includes(roleId));
          }
    ...
    let paginatedItems: typeof announcementsWithReadStatus;
...
export async function updateAnnouncement(req: Request, res: Response) {
  try {
    ...
    if (data.status) {
      ...
      status = data.status;
    } else if ((data as any).publish === true) {
    ...
    const updateData: any = {
      ...
      ...(data.publishAt !== undefined || (data as any).publish === true 
        ? { publishAt: publishAtDate } 
        : {}),
      ...(data.expireAt !== undefined && { expireAt: data.expireAt ? new Date(data.expireAt) : null }),
      ...(data.target && { target: data.target as any }),
 ...
 */
export async function acknowledgeAnnouncement(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const currentUserId = getCurrentUserId(req);
...
export async function listAnnouncementReads(req: Request, res: Response) {
  try {
    ...
    const where: Prisma.AnnouncementReadWhereInput = {
      announcementId: id,
    };
 ...
 */
export async function getAnnouncementStats(req: Request, res: Response) {
  try {
    const { id } = req.params;
