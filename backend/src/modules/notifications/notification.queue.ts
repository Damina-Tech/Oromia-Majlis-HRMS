...
function getNotificationQueueEvents(): QueueEvents | null {
...
async function processNotificationDeliveryDirectly(deliveryId: string) {
  ...
  try {
    ...
    switch (delivery.channel) {
      ...
      default:
        console.log(
          `🔔 [Notification] In-app delivery for ${recipient.id}: ${notification.title}`
        );
        break;
    }
...
export async function startNotificationWorker() {
  ...
  if (!conn) {
    console.log(
      "ℹ️  Redis is not available; notifications will be processed directly."
    );
