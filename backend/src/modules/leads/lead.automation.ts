...
const FOLLOW_UP_WINDOWS: Record<LeadStage, number> = {
  NEW: 6,
  CONTACTED: 12,
  QUALIFIED: 24,
  ENGAGED: 24,
...
const PRIORITY_MAP: Record<LeadPriority, TaskPriority> = {
...
export class LeadAutomationService {
  ...
  static groupByStage<T extends { stage: LeadStage }>(leads: T[]) {
    return Object.values(LeadStage).reduce<Record<LeadStage, T[]>>((acc, stage) => {
      acc[stage] = leads.filter((lead) => lead.stage === stage);
      return acc;
    }, {} as Record<LeadStage, T[]>);
  }
