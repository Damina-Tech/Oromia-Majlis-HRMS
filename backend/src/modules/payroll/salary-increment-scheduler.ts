...
export interface IncrementConfig {
  ...
  reason?: string;
  notes?: string;
  employeeIds?: string[]; // If provided, only apply to these employees
  departmentId?: string; // If provided, only apply to this department
}
 ...
 */
export async function runAnnualIncrementJob(config: IncrementConfig): Promise<{
  successful: number;
  failed: number;
 ...
 * });
 */
export function scheduleAnnualIncrement(
  cronExpression: string,
  config: IncrementConfig,
  cronLib?: any // Accept any cron library (node-cron, cron, etc.)
) {
  if (!cronLib) {
    console.warn(
      "No cron library provided. Please install node-cron or similar and pass it to scheduleAnnualIncrement."
    );
    return null;
  }
