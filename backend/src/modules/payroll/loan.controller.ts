...
export async function updateLoan(req: Request, res: Response) {
  try {
    ...
    if (dto.interestRate !== undefined) updateData.interestRate = new Prisma.Decimal(dto.interestRate);
    if (dto.monthlyPayment !== undefined) updateData.monthlyPayment = new Prisma.Decimal(dto.monthlyPayment);
...
export async function addLoanRepayment(req: Request, res: Response) {
  try {
    const { id } = req.params;
