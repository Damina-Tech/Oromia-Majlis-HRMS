...
import { BatchGenerateIdCardDto, CreateIdCardTemplateDto, GenerateIdCardDto, IdCardTemplateSettingsSchema, UpdateIdCardTemplateDto, } from "./employee.dto.js";
...
const canManageAll = (user) => Boolean(user?.permissions?.includes("employees.id.manage") || user?.permissions?.includes("employees.write"));
const hasGeneratePermission = (user) => Boolean(user?.permissions?.includes("employees.id.generate"));
...
async function ensureRequesterEmployee(user) {
    if (!user?.employeeId)
        return null;
...
function assertDepartmentAccess(employee, requesterEmployee, user) {
...
async function recordGeneration({ employeeId, templateId, generatedById, pdfUrl, pngUrl, metadata, mode, zipUrl, }) {
...
async function zipPdfFiles(entries) {
...
export async function listIdCardTemplates(req, res) {
    const templates = await prisma.employeeIdTemplate.findMany({
        orderBy: [{ isDefault: "desc" }, { createdAt: "desc" }],
    });
    res.json(templates);
}
export async function createIdCardTemplate(req, res) {
...
export async function updateIdCardTemplate(req, res) {
...
export async function deleteIdCardTemplate(req, res) {
    const { templateId } = req.params;
...
export async function setDefaultIdCardTemplate(req, res) {
    const { templateId } = req.params;
    const template = await prisma.$transaction(async (tx) => {
        await tx.employeeIdTemplate.updateMany({ data: { isDefault: false } });
        return tx.employeeIdTemplate.update({
...
export async function generateEmployeeIdCard(req, res) {
    const { id } = req.params;
...
export async function batchGenerateEmployeeIdCards(req, res) {
...
//# sourceMappingURL=employee-id.controller.js.map