 ...
 */
export async function enqueueAnnouncementDelivery(announcementId) {
    try {
        ...
        if (userIds.length > 0) {
            try {
                await NotificationService.sendNotification({
                    ...
                    message: announcement.type === "MEETING"
            ...
            for (const targetUser of targetUsers) {
                ...
                if (channel !== "IN_APP") {
                    if (channel === "EMAIL" && !targetUser.email)
                        continue;
                    if ((channel === "SMS" || channel === "WHATSAPP") && !targetUser.phone)
                        continue;
                }
 ...
 */
export async function processDeliveryQueue(announcementId) {
            ...
            case "EMAIL":
                if (delivery.recipient) {
                    try {
                        ...
                        providerRef = await sendAnnouncementEmail(delivery.recipient, delivery.announcement.title, delivery.announcement.body, delivery.announcement.id);
                        success = true;
                    }
                    catch (emailError) {
            ...
            case "SMS":
                if (delivery.recipient) {
                    try {
                        ...
                        providerRef = await sendAnnouncementSMS(delivery.recipient, delivery.announcement.title, delivery.announcement.id);
                        success = true;
                    }
                    catch (smsError) {
            ...
            case "WHATSAPP":
                if (delivery.recipient) {
                    try {
                        ...
                        providerRef = await sendAnnouncementWhatsApp(delivery.recipient, delivery.announcement.title, delivery.announcement.body, delivery.announcement.id);
                        success = true;
                    }
                    catch (whatsappError) {
 ...
 */
export async function retryFailedDeliveries(announcementId) {
    const where = {
        status: "FAILED",
    };
    if (announcementId) {
