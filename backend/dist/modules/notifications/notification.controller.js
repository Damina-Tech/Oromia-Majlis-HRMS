import { NotificationModule, NotificationType, NotificationDeliveryStatus, } from "@prisma/client";
import prisma from "../../db/client.js";
import { ListNotificationsQuerySchema, MarkNotificationsReadDto, SendTestNotificationDto, UpdateNotificationPreferencesDto, } from "./notification.dto.js";
...
export async function markNotifications(req, res) {
    try {
        const userId = getCurrentUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
...
export async function getDeliveryLogs(req, res) {
    try {
        ...
        if (query.module) {
            where.notification = { module: query.module };
        }
        const skip = (query.page - 1) * query.pageSize;
        const take = query.pageSize;
        const [items, total] = await Promise.all([
...
export async function resendNotificationDelivery(req, res) {
    try {
        const userId = getCurrentUserId(req);
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
