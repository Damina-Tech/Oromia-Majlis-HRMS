    ...
    try {
        ...
        const asset = await prisma.$transaction(async (tx) => {
            ...
            if (data.purchasePrice !== undefined)
                updateData.purchasePrice = data.purchasePrice ? new Prisma.Decimal(data.purchasePrice) : null;
            if (data.currency)
                updateData.currency = data.currency;
            if (data.vendorId !== undefined)
                updateData.vendorId = data.vendorId || null;
            if (data.warrantyUntil)
                updateData.warrantyUntil = new Date(data.warrantyUntil);
            ...
            if (data.depreciationMethod !== undefined)
                updateData.depreciationMethod = data.depreciationMethod || null;
            if (data.depreciationRate !== undefined)
                updateData.depreciationRate = data.depreciationRate ? new Prisma.Decimal(data.depreciationRate) : null;
            if (data.lifeYears !== undefined)
                updateData.lifeYears = data.lifeYears ? new Prisma.Decimal(data.lifeYears) : null;
 ...
 */
export async function revokeAsset(req, res) {
    try {
        const { id } = req.params;
...
export async function getAssetStats(req, res) {
    try {
        ...
        const [totalAssets, statusCounts, conditionCounts, categoryBreakdown, locationBreakdown, totalValue, assignedCount, maintenanceCount, disposedCount,] = await Promise.all([
            prisma.asset.count({ where }),
        ...
        const locationIds = locationBreakdown.map((l) => l.locationId).filter(Boolean);
 ...
 * Bulk update assets
 */
export async function bulkUpdateAssets(req, res) {
    try {
