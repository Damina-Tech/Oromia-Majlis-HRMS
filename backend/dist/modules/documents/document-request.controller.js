...
import { CreateDocumentRequestDto, UpdateDocumentRequestDto, ListDocumentRequestsQuery, } from "./document-request.dto.js";
...
export async function listDocumentRequests(req, res) {
    try {
        ...
        const isAdminOrHR = currentUser?.userRoles.some((ur) => ur.role.name.toUpperCase() === "ADMIN" || ur.role.name.toUpperCase() === "HR") || false;
        const where = {};
 ...
 */
export async function getDocumentRequest(req, res) {
    try {
        const { id } = req.params;
        const currentUserId = getCurrentUserId(req);
...
export async function updateDocumentRequest(req, res) {
    try {
        ...
        if (data.status === "GENERATED" && existingRequest.status === "APPROVED") {
            try {
                // Process template
                const processedContent = await processTemplate(existingRequest.template.content, existingRequest.employeeId, undefined, false);
                // Generate PDF
                const fileName = generateDocumentFileName(existingRequest.template.code, existingRequest.employee.employeeCode);
                ...
                try {
                    const fullFilePath = pdfResult.filePath.startsWith("/")
                        ...
                        : pdfResult.filePath;
                    const absolutePath = path.join(__dirname, "../../../", fullFilePath);
                    await sendGeneratedDocument(existingRequest.employee.email, `${existingRequest.employee.firstName} ${existingRequest.employee.lastName}`, generatedDoc.fileName, existingRequest.template.name, absolutePath, generatedDoc.fileName);
                }
                catch (emailError) {
        ...
        try {
            const recipientUserId = existingRequest.employee?.user?.id ?? updatedRequest.employee?.user?.id;
...
//# sourceMappingURL=document-request.controller.js.map