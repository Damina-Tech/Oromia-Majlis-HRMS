...
import { CreateDocumentTemplateDto, UpdateDocumentTemplateDto, ListTemplatesQuery, GenerateDocumentDto, PreviewDocumentDto, ListGeneratedDocumentsQuery, } from "./document.dto.js";
...
export async function previewTemplate(req, res) {
    try {
        ...
        const processedContent = await processTemplate(template.content, data.employeeId, data.mergeData, data.useSampleData || !data.employeeId);
        return res.status(200).json({
...
export async function generateDocument(req, res) {
    try {
        ...
        for (const employeeId of employeeIds) {
            // Process template
            const processedContent = await processTemplate(template.content, employeeId, data.mergeData, false);
            ...
            // Generate PDF
            const fileName = generateDocumentFileName(template.code, employee?.employeeCode || null);
            ...
            if (data.email && employee && employee.email) {
                try {
                    ...
                    await sendGeneratedDocument(employee.email, `${employee.firstName} ${employee.lastName}`, generatedDoc.fileName, template.name, fullFilePath, generatedDoc.fileName);
 ...
 */
export async function listGeneratedDocuments(req, res) {
    try {
 ...
 */
export async function getMergeFields(req, res) {
    try {
        const mergeFields = {
            employee: {
