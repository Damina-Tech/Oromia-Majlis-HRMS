...
import { CreateLeaveRequestDto, UpdateLeaveStatusDto, UpdateLeaveRequestDto, ListLeaveRequestsQuery, } from "./leave.dto.js";
    ...
    try {
        ...
        if (balances.length === 0) {
            const defaultBalances = [
                { leaveType: "CASUAL", allocatedDays: 12 },
                { leaveType: "SICK", allocatedDays: 10 },
                { leaveType: "VACATION", allocatedDays: 21 },
                { leaveType: "PERSONAL", allocatedDays: 5 },
            ];
            await prisma.$transaction(defaultBalances.map((defaultBalance) => prisma.leaveBalance.create({
                data: {
                    employeeId,
