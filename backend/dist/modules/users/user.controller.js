...
import { CreateUserDto, UpdateUserDto, ListUsersQuery, UpdateSelfDto, UpdateUserPermissionsDto, } from "./user.dto.js";
...
export async function listUsers(req, res) {
    try {
        ...
        if (search) {
            ...
            allItems = allItems.filter((item) => item.firstName.toLowerCase().includes(s) ||
                item.lastName.toLowerCase().includes(s) ||
                item.email.toLowerCase().includes(s));
        }
        // Apply role filter (only affects real users)
        if (roleId && roleId !== "all") {
            allItems = allItems.filter((item) => item._isUserAccount && item.roles.some((r) => r.id === roleId));
        }
        // Apply status filter
        if (status) {
...
export async function getUser(req, res) {
    try {
        ...
        const permissionsWithDetails = buildEffectivePermissionDetails(user.userRoles, user.userPermissions ?? []);
        ...
        const user = await prisma.$transaction(async (tx) => {
            ...
            if (updatedUser.employee) {
                ...
                if (dto.firstName !== undefined)
                    employeeProfileUpdates.firstName = dto.firstName;
                if (dto.lastName !== undefined)
                    employeeProfileUpdates.lastName = dto.lastName;
                if (dto.avatarUrl !== undefined)
                    employeeProfileUpdates.avatarUrl = dto.avatarUrl;
...
export async function updateCurrentUser(req, res) {
    try {
        ...
        if (dto.firstName || dto.lastName || dto.avatarUrl) {
            ...
            if (dto.firstName)
                employeeUpdate.firstName = dto.firstName;
            if (dto.lastName)
                employeeUpdate.lastName = dto.lastName;
            if (dto.avatarUrl)
                employeeUpdate.avatarUrl = dto.avatarUrl;
...
export async function getUserPermissions(req, res) {
    try {
        ...
        const effectivePermissions = buildEffectivePermissionDetails(user.userRoles, user.userPermissions);
        return res.json({
