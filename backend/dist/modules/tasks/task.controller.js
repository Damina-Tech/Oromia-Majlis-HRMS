...
import { CreateTaskDto, UpdateTaskDto, ListTasksQuery, AddCommentDto, AddTimeLogDto, BulkUpdateTasksDto, } from "./task.dto.js";
...
function checkTaskModelAvailable(res) {
    if (!prisma.task) {
        res.status(500).json({
...
export async function createTask(req, res) {
    try {
        if (!checkTaskModelAvailable(res))
            return;
        ...
        if (data.assigneeIds && data.assigneeIds.length > 0) {
            await Promise.all(data.assigneeIds.map((employeeId) => prisma.taskAssignment.create({
                data: {
                    taskId: task.id,
                    employeeId,
        ...
        if (data.watcherIds && data.watcherIds.length > 0) {
            await Promise.all(data.watcherIds.map((userId) => prisma.taskWatcher.create({
                data: {
                    taskId: task.id,
                    userId,
                },
    ...
    try {
        ...
        if (!isAdmin && !isHR && !isManager) {
            if (currentUserEmployeeId) {
                ...
                accessConditions.push({ assignments: { some: { employeeId: currentUserEmployeeId } } }, { watchers: { some: { userId: currentUserId } } }, { createdBy: currentUserId });
            }
            else {
                // User without employeeId can only see tasks they're watching or created
                accessConditions.push({ watchers: { some: { userId: currentUserId } } }, { createdBy: currentUserId });
            }
        }
    ...
    try {
        ...
        try {
            ...
            if (statusChanged && assignmentUserIds.length + watcherUserIds.length > 0) {
                const recipients = Array.from(new Set([...assignmentUserIds, ...watcherUserIds]));
                await NotificationService.sendNotification({
                    module: NotificationModule.TASK,
            ...
            if (Array.isArray(data.assigneeIds)) {
                const previousEmployeeIds = new Set((existingTask.assignments || []).map((assignment) => assignment.employeeId));
                const addedEmployeeIds = data.assigneeIds.filter((employeeId) => !previousEmployeeIds.has(employeeId));
...
export async function getTaskStats(req, res) {
    try {
        ...
        res.json({
            ...
            tasksByUser: await Promise.all(byUser.map(async (u) => {
