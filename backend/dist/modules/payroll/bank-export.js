 ...
 */
export async function generateBankExport(payrollRunId, format = "csv") {
 ...
 */
function generateTextExport(rows) {
    ...
    lines.push("");
    // Column headers
    lines.push("Employee Name".padEnd(30) +
    ...
    // Data rows
    rows.forEach((row) => {
        lines.push(row.employeeName.padEnd(30) +
 ...
 */
export async function getBankExportBuffer(payrollRunId, format = "csv") {
    const content = await generateBankExport(payrollRunId, format);
    return Buffer.from(content, "utf-8");
}
//# sourceMappingURL=bank-export.js.map