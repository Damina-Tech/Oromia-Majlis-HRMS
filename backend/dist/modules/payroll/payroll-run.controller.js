...
import { CreatePayrollRunDto, UpdatePayrollRunDto, ReviewPayrollRunDto, ApprovePayrollRunDto, ProcessPayrollRunDto, UpdatePayrollItemDto, ListPayrollRunQuery, ListPayrollItemsQuery, } from "./payroll-run.dto.js";
import { calculatePayrollItem, } from "./payroll-calculator.js";
import { createAuditLog } from "./payroll-audit.js";
import { generateBankExport } from "./bank-export.js";
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const employee of employees) {
                try {
                    const calculation = await calculatePayrollItem(employee.id, periodStart, periodEnd);
        ...
        try {
            const periodLabel = `${format(new Date(dto.periodStart), "MMM dd, yyyy")} - ${format(new Date(dto.periodEnd), "MMM dd, yyyy")}`;
 ...
 * Review payroll run
 */
export async function reviewPayrollRun(req, res) {
    try {
        const { id } = req.params;
 ...
 */
export async function listPayrollItems(req, res) {
    try {
 ...
 */
export async function getPayrollItem(req, res) {
    try {
        const { id } = req.params;
...
export async function exportBankFile(req, res) {
    try {
        ...
        const userId = req.user?.id;
        const fileFormat = (format === "txt" ? "txt" : "csv");
...
export async function downloadPayslip(req, res) {
    try {
        ...
        res.setHeader("Content-Disposition", `attachment; filename="payslip_${payrollItem.id}.pdf"`);
