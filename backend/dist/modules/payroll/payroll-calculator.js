 ...
 */
export async function calculateIncomeTax(grossSalary, year) {
    ...
    if (grossSalary <= 5250)
        return 337.50 + (grossSalary - 3200) * 0.20;
    if (grossSalary <= 7800)
        return 747.50 + (grossSalary - 5250) * 0.25;
 ...
 */
export async function calculatePension(grossSalary, year) {
    const currentYear = year || new Date().getFullYear();
 ...
 */
export async function calculateAllowances(employeeId, periodStart, periodEnd, basicSalary) {
 ...
 */
export async function calculateLoanDeductions(employeeId, payrollRunId) {
 ...
 */
export async function calculateAdvanceDeductions(employeeId, payrollRunId) {
 ...
 */
export function calculateAbsenceDeductions(basicSalary, workingDays, absentDays) {
    if (workingDays === 0)
        return 0;
    const dailyRate = basicSalary / workingDays;
    return dailyRate * absentDays;
}
export async function calculatePayrollItem(employeeId, periodStart, periodEnd, options) {
    ...
    const healthInsurance = (options?.healthInsuranceRate || 0.02) * grossSalary; // Default 2%
